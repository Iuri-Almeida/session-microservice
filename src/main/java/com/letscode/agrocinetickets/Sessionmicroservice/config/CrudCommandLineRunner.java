...
public class CrudCommandLineRunner implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        listCompletableFuture.thenAccept(tables -> {
            if (tables != null && !tables.contains(Session.class.getSimpleName().toLowerCase())) {
                DynamoDbAsyncTable<Session> sessionDynamoDbAsyncTable = enhancedAsyncClient.table(Session.class.getSimpleName().toLowerCase(), TableSchema.fromBean(Session.class));
                sessionDynamoDbAsyncTable.createTable();
            }
        });
    }
}
